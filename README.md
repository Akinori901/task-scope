...
AI 生成時、対象リポジトリの `.claude/skills/` にあるスキルを自動検出し、
プロンプトに一覧を添えて「合致するものがあれば実行し、その手順・出力形式に従う」よう
指示します。報告書や調査の様式はプロジェクトごとに異なるため、**生成種別 → スキル名の
固定表は持たず**、実在するスキルを都度拾う方式です。

- リポジトリ側にスキルを追加すれば、task-scope は無改修で拾う
- 親ディレクトリ（最大3階層）も探索するので、モノレポの共通スキルも利用できる
  （同名なら近い階層＝リポジトリ固有のものを優先）
- 合致するスキルが無ければ通常どおりコードを読んで生成する
